      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-1">Create and manage groups, assign group admins</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-gray-900 border border-gray-700">
            <div className="px-6 py-4 border-b border-gray-700">
              <h3 className="text-xl font-semibold text-white">ALL GROUPS ({groups.length})</h3>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-800">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-300 uppercase tracking-wider">Group Name</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-300 uppercase tracking-wider">Start Date</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-300 uppercase tracking-wider">Group Admin</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-300 uppercase tracking-wider">Members</th>
                ...
                <tbody className="bg-gray-900 divide-y divide-gray-700">
                  {groups.map((group) => (
                    <tr key={group.id} className="hover:bg-gray-800 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-semibold text-white">{group.name}</div>
                        <div className="text-xs text-gray-400">ID: {group.id.slice(0, 8)}...</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                        {new Date(group.start_date).toLocaleDateString()}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-300">{group.admin_email}</div>
                        <div className="text-xs text-gray-400">ID: {group.admin_id.slice(0, 8)}...</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="inline-flex items-center px-3 py-1 text-xs font-bold bg-blue-600 text-white border border-blue-500">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-3">
                          ...
                          <button
                            ...
                            className="bg-green-600 text-white px-3 py-1 hover:bg-green-500 transition-colors border border-green-500"
                          >
                            Members
                          </button>
                          <button
