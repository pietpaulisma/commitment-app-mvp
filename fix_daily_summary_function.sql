-- Fix the generate_daily_summary function to properly fetch user data
...
BEGIN
  ...
  -- Count members who worked out today using workout_logs table
  SELECT COUNT(DISTINCT wl.user_id) INTO v_committed_members
  ...
  SELECT p.username, COUNT(wl.id) as workout_count
  ...
  ORDER BY workout_count DESC, wl.logged_at DESC
  LIMIT 1;
